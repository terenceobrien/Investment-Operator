...
class Frbus:
    def __init__(self, filepath: str, mce: Optional[str] = None):
        ...
        self.orig_exo_names: List[str] = (
            tmp_exos
            + [endo + "_aerr" for endo in self.endo_names]
            + [endo + "_trac" for endo in self.endo_names]
        )
        self.exo_names = deepcopy(self.orig_exo_names)
        # Drop unused series again, in case of unused aerrs or etc.
        self.exo_names = [
            exo
            for exo in self.exo_names
            if any([re.search(rf"\b{exo}\b", eq) for eq in eqs])
        ]
    ...
    def append_replace(self, eqs_map: Dict[str, str]) -> None:
        ...
        tokens = flatten(
            [re.sub(r"[^A-Za-z0-9_\.]", " ", eq).split() for eq in eqs_map.values()]
        )
        ...
        self.exo_names = [
            ...
            for exo in self.exo_names
            if any(
                [re.search(rf"\b{exo}\b", eq) for eq in lexing.to_eqs(self.lexed_eqs)]
            )
        ]
        ...
        options: Optional[Dict]
            Options to pass to solver:
                ...
                ``force_recompute: bool``
                    ...
                    Defaults to ``False``.
                ``try_umfpack: bool``
                    When set to ``True``, use ``scikit_umfpack`` with either the Newton
                    or trust-region solver, if ``scikit_umfpack`` is also installed. Set
                    to ``False`` if ``scikit_umfpack`` is installed and causing issues.
                    Defaults to ``True``.
