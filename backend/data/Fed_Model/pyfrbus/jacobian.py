...
def mce_create_jacobian(
    ...
    for i in range(len(exprs)):
        ...
        eq = exprs[i]
        rhs_vars_idxs = [int(re.findall(r"\d+", var)[0]) for var in rhs_vars[i]]

        # Partial of x[i] with respect to itself
        deriv = take_partial(
            i, i, eq, data_hash, jac_dict, offsets, col_to_endo, rhs_vars_idxs
        )
        jac_list += [(i, i, deriv)]
        jac_dict[(i, i)] = deriv
        ...
        for j in rhs_vars_idxs:
            deriv = take_partial(
                i, j, eq, data_hash, jac_dict, offsets, col_to_endo, rhs_vars_idxs
            )
            jac_list += [(i, j, deriv)]
            jac_dict[(i, j)] = deriv
...
def take_partial(
    ...
    # BUT we don't want to do this for equations that may involve a terminal
    # so we skip if any rhs_vars_idxs are within offset[0] of the terminal length
    if (
        i > offsets[0]
        and j > offsets[0]
        and (
            (not rhs_vars_idxs) or (max(rhs_vars_idxs) + offsets[0]) < len(col_to_endo)
        )
    ):
