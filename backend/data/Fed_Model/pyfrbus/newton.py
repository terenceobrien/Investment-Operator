...
# Attempt to import scikit_umfpack, if available
try:
...
def newton(
    ...
    call_jac: Callable[[ndarray, ndarray, ndarray], csc_matrix],
    guess: ndarray,
    vals: ndarray,
    solution: ndarray,
    options: Dict,
    ...
    try_umfpack: bool = options["try_umfpack"]
    ...
    scale = get_preconditioner(jac) if precond else identity(jac.shape[0], format="csc")
    # Compute LU decomposition
    lu = umfpack.splu(scale @ jac) if _UMFPACK and try_umfpack else splu(scale @ jac)
    ...
    for iter in range(maxiter):
        ...
        if norm(fun_val_tmp) > last_resid * 0.5 or force_recompute:
            ...
            scale = (
                ...
                if precond
                else identity(jac.shape[0], format="csc")
            )
            # Compute new LU decomposition
            lu = (
                umfpack.splu(scale @ jac)
                if _UMFPACK and try_umfpack
                else splu(scale @ jac)
            )
...
def trust(call_fun, call_jac, guess, vals, solution, options: Dict):
    ...
    for iter in range(maxiter):
        ...
        jac = call_jac(guess, vals, solution)
        p = dogleg(fun_val, jac, radius, precond, try_umfpack, debug)
...
def dogleg(fun_val, jac, radius, precond, try_umfpack, debug) -> ndarray:
    p: ndarray = cauchy_point(fun_val, jac, radius)
    if norm(p) == radius:
        return p
    else:
        scale = (
            get_preconditioner(jac) if precond else identity(jac.shape[0], format="csc")
        )
        with warnings.catch_warnings():
            if not debug:
                warnings.simplefilter("ignore")
            z: ndarray = (
                umfpack.spsolve(scale @ jac, scale @ -fun_val)
                if _UMFPACK and try_umfpack
                else spsolve(scale @ jac, scale @ -fun_val)
            )
...
def get_preconditioner(jac) -> csc_matrix:
    return csc_matrix(
        (
