...
from pyfrbus.exceptions import InvalidModelError


# Validate equation formatting, check for common mistakes
def validate_equation(eq: str):
    ...
    for char in eq:
        ...
        if char == ")":
            p_count -= 1
        if p_count < 0:
            raise InvalidModelError(f"unbalanced parenthesis in equation: {eq}")
    if p_count > 0:
        raise InvalidModelError(f"unbalanced parenthesis in equation: {eq}")
    # Check for ^ instead of **
    if "^" in eq:
        raise InvalidModelError(f"'^' used instead of '**' for power in: {eq}")
...
def lex_eq(eq: str) -> List[Tuple[str, Optional[Tuple[str, int]]]]:
    # First, validate equation to avoid throwing weird errors later
    ...
    regex = r"(.*?)\b([A-Za-z][A-Za-z0-9_\.]*)(?:\((-?\d+)\))?(?!\w)(.*)"
