...
for declaration in constants.CONST_SUPPORTED_FUNCTIONS_SYMEX_DEC:
    exec(declaration, sys._getframe().f_globals, sys._getframe().f_locals)
...
def eval_jac(
...
) -> Callable[[ndarray, ndarray, Optional[ndarray]], Union[ndarray, csc_matrix]]:
    ...
    if sparse:
        ...
        def e_j_sparse(
            ...
            return csc_matrix((mat, (row, col)), shape=(size, size))
