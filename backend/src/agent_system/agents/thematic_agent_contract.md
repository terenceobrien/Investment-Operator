...
**Rule TM-1 (Consensus claim categorization).** When asserting a consensus_view for a candidate, the agent must indicate which type of consensus is being claimed: (a) estimate_consensus — quantitative claims about sell-side EPS, interest expense, revenue, or other modeled values; these claims require a data source, (b) positioning_consensus — claims about where money is allocated, including fund flows, short interest, options positioning, or institutional holdings; these claims require a data source, or (c) narrative_consensus — qualitative claims about discourse, what the market is "talking about" or "focused on"; these are LLM-derived priors and require explicit acknowledgment. Currently no consensus data sources are available to the agent. All consensus claims must therefore be marked with consensus_type and all estimate/positioning/mixed claims must include an explicit verification_required entry in the candidate's fit_evidence describing what source would be needed.

**Rule TA-5 (Selection logic must be auditable).** The agent must populate (a) mapping_logic with prose explaining the high-level selection rationale, (b) excluded with at least 2-3 ExclusionRecord entries naming candidates considered and rejected with specific reasons, (c) rejected_quick with up to 30 quickly dismissed tickers and tight one-line reasons, and (d) universe_considered with a rough count of candidates evaluated before narrowing. This structurally enforces that the agent considered alternatives rather than just listing what came to mind.
...
**Rule TA-14 (Fit strength reflects real tie to the priority).** fit_strength is computed from four explicit fit_strength_components: thesis_mechanism_match (40%), consensus_anchoring_strength (30%), catalyst_proximity (20%), and tradeability (10%). Each component is scored 0.0-1.0 using the prompt's documented rubric. The agent must populate the components; the system computes fit_strength deterministically. Candidates with genuine computed fit_strength below approximately 0.4 probably do not belong in the candidate set — they should be exclusions instead. The 0.4 threshold is a soft guideline, not a hard rule, but agents producing many sub-0.4 candidates are likely padding the map.
...
The agent should consider thesis-specific catalysts before defaulting to earnings. For refinancing/EPS-drag theses, this means debt tender offers, bond pricings, credit agreement amendments, ratings actions, and refinancing announcements. For margin inflection, this means pricing/cost spread announcements, volume/utilization disclosures, and raw-material updates. For multiple expansion, this means ratings upgrades, index inclusions, and analyst day outreach. For regulatory theses, this means specific FDA/FTC/court dates or comment periods. Earnings should be used only if it is likely to address the thesis mechanism or if no thesis-specific catalyst exists inside the relevant horizon.
...
- TA-S: Missing consensus categorization — consensus_view is populated but consensus_type is missing, wrong, or estimate/positioning/mixed claims lack verification_required fit_evidence. (Caught by TM-1; checkable structurally.)

- TA-T: Invisible quick rejects — universe_considered is materially larger than candidates + excluded, but rejected_quick is empty or vague. (Caught by TA-5; checkable by reviewing rejected_quick specificity.)

- TA-U: Opaque fit score — fit_strength appears without populated fit_strength_components or differs from the weighted component computation. (Caught by TA-14; checkable structurally.)

## Versioning

This contract is v3. Material additions include TM-1 consensus categorization, rejected_quick universe persistence, thesis-specific catalyst priority, and fit_strength_components with deterministic fit_strength computation. Future material changes to rule semantics require bumping the contract version. The version should be referenced in commit messages whenever the contract changes. Contract version and prompt version are tracked separately; see `thematic_agent_prompts.py` constants.
