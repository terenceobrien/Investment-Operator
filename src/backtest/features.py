...
from src.state.market_state import MarketState  # your existing dataclass
from src.state.scoring import score_market_state  # your existing scorer
...
def build_daily_feature_frame(
    ...
    tickers = sorted(set(cross_assets + sectors))
    ...
    spy_vol_vs_20d_pct = (spy_vol / spy_vol_ma - 1.0) * 100.0
    ...
    out["fwd_ret_oc_1d"] = (spy_close / spy_open - 1.0) * 100.0  # open->close same day
    out["fwd_ret_cc_1d"] = (spy_close.shift(-1) / spy_close - 1.0) * 100.0  # close->next close
...
def score_history_both_signal_times(
    ...
    def build_state_from_row(dt: pd.Timestamp, row: pd.Series) -> MarketState:
        ...
        for col in features.columns:
            if col.startswith("ret_") and col.endswith("_1d"):
                # ret_TICKER_1d
                ticker = col[len("ret_") : -len("_1d")]
                v = row.get(col)
                if pd.notna(v):
                    cross_asset_returns[ticker] = float(v)
        ...
        try:
            state = replace(
                ...
                spy_above_prev_close=(bool(row.get("spy_close") > row.get("spy_prev_close"))
                                      if pd.notna(row.get("spy_close")) and pd.notna(row.get("spy_prev_close")) else None),
                spy_vwap=None,
                spy_above_vwap=None,
    ...
    def score_for_index(ix: pd.DatetimeIndex) -> pd.DataFrame:
        rows = []
        for dt in ix:
            row = features.loc[dt]
    ...
    scored_open = score_for_index(feat_open.dropna().index)
    scored_open = scored_open.reindex(features.index)  # align to original dates
    scored_open["signal_time"] = "open"

    # --- signal_time = close ---
    scored_close = score_for_index(features.dropna().index)
    scored_close = scored_close.reindex(features.index)
    scored_close["signal_time"] = "close"

    # Merge into a single wide frame
    def add_fwd(df: pd.DataFrame, label: str) -> pd.DataFrame:
        out = df.copy()
        out["fwd_ret_oc_1d"] = features["fwd_ret_oc_1d"]
        out["fwd_ret_cc_1d"] = features["fwd_ret_cc_1d"]
        out.columns = pd.MultiIndex.from_product([[label], out.columns])
        return out

    wide = pd.concat([add_fwd(scored_open, "open"), add_fwd(scored_close, "close")], axis=1)
    return wide
