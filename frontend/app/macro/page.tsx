...
import { T, sx, formatAccountingPct } from '@/lib/tokens';
...
function signalColor(sig: string): string {
  return sig === 'bullish' ? T.up : sig === 'bearish' ? T.dn : T.wa;
}
...
const STANCE_COLOR: Record<string, string> = {
  bullish: T.up, bearish: T.dn, neutral: T.wa, cautious: T.wa, mixed: T.wa,
};

// ─────────────────────────────────────────────────────────────
// Small shared primitives (local; styled to match sx panels)
// ─────────────────────────────────────────────────────────────
function Panel({ title, meta, children, prominent }: { title?: string; meta?: string; children: React.ReactNode; prominent?: boolean }) {
  return (
    <section style={{ ...sx.panel, boxShadow: prominent ? T.shadowSoft : sx.panel.boxShadow }}>
      {title ? (
...
function Chip({ label, color }: { label: string; color?: string }) {
  return (
    <span style={{
      ...
      fontFamily: T.sans, fontSize: '11px', fontWeight: 650, letterSpacing: '0.02em',
      background: color ? `${color}1A` : T.surfaceMuted,
      color: color ?? T.textMuted, border: `1px solid ${color ? `${color}33` : T.borderSub}`,
    }}>{label}</span>
  );
}
function Eyebrow({ children }: { children: React.ReactNode }) {
  return <div style={{ fontFamily: T.mono, fontSize: '12px', letterSpacing: '0.2em', color: T.textMuted, marginBottom: '10px' }}>{children}</div>;
}
function MutedLabel({ children }: { children: React.ReactNode }) {
  return <div style={{ fontFamily: T.sans, fontSize: '10px', letterSpacing: '0.12em', textTransform: 'uppercase', color: T.textMuted, fontWeight: 700, marginBottom: '8px' }}>{children}</div>;
}
  ...
  return (
    <Panel title="Market pulse" meta={`${f.horizon.toUpperCase()} · ${f.probMode.replace(/_/g, ' ')}`} prominent>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: '14px', flexWrap: 'wrap' }}>
        <span style={{ fontFamily: T.sans, fontSize: '24px', fontWeight: 650, color: T.text }}>
          {f.confLevel === 'high' ? 'Constructive, high conviction' : f.confLevel === 'low' ? 'Constructive, selective' : 'Mixed, watchful'}
        </span>
        <Chip label={`${pct1(f.dominantProb)} dominant`} color={T.accent} />
      </div>
      {f.composite !== null ? (
        <div style={{ display: 'flex', alignItems: 'baseline', gap: '12px', margin: '14px 0 4px' }}>
          <span style={{ ...sx.monoVal, fontSize: '40px', color: T.text }}>{f.composite.toFixed(1)}</span>
          <span style={{ fontFamily: T.sans, fontSize: '13px', fontWeight: 600, color: T.accentDark }}>composite regime score</span>
        </div>
      ) : null}
      <p style={{ margin: '16px 0 0', paddingTop: '16px', borderTop: `1px solid ${T.borderSub}`, fontFamily: T.sans, fontSize: '14px', color: T.textSub, lineHeight: 1.6 }}>
...
function CurrentRegime({ f }: { f: Forecast }) {
  return (
    <Panel title="Current regime">
      <div style={{ fontFamily: T.sans, fontSize: '20px', fontWeight: 650, color: T.text, marginBottom: '8px' }}>{f.dominantLabel}</div>
      <p style={{ margin: '0 0 20px', fontFamily: T.sans, fontSize: '13px', color: T.textSub, lineHeight: 1.55 }}>{f.regimeRead}</p>
      {f.layers.map((l) => (
        <div key={l.layer} style={{ marginBottom: '14px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}>
            <span style={{ fontFamily: T.sans, fontSize: '13px', fontWeight: 600, color: T.text }}>{l.name}</span>
            <span style={{ fontFamily: T.mono, fontSize: '12px', color: T.textMuted }}>{l.score.toFixed(1)} · {l.trend}</span>
          </div>
          <div style={{ height: '5px', background: T.borderSub, borderRadius: '3px', overflow: 'hidden' }}>
            <div style={{ height: '100%', width: `${Math.min(100, l.score * 10)}%`, background: signalColor(l.signal), borderRadius: '3px' }} />
          </div>
        </div>
      ))}
    </Panel>
...
function DominantBanner({ f }: { f: Forecast }) {
  return (
    <section style={{ ...sx.panel, background: T.navy, border: `1px solid ${T.navySoft}` }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', gap: '28px', padding: '30px 32px', flexWrap: 'wrap' }}>
        <div style={{ flex: '1 1 520px' }}>
          <div style={{ fontFamily: T.mono, fontSize: '11px', letterSpacing: '0.16em', color: 'rgba(198,210,224,0.6)' }}>DOMINANT SCENARIO</div>
          <h2 style={{ fontFamily: T.sans, fontSize: '27px', fontWeight: 650, color: '#fff', lineHeight: 1.15, margin: '12px 0 12px', maxWidth: '760px' }}>{f.headline}</h2>
          <p style={{ margin: 0, fontFamily: T.sans, fontSize: '14px', color: 'rgba(198,210,224,0.85)', lineHeight: 1.55, maxWidth: '740px' }}>{f.summary}</p>
        </div>
        <div style={{ textAlign: 'right', minWidth: '180px' }}>
          <div style={{ fontFamily: T.mono, fontSize: '11px', letterSpacing: '0.14em', color: 'rgba(198,210,224,0.6)' }}>CONFIDENCE · {f.confLevel.toUpperCase()}</div>
          <div style={{ ...sx.monoVal, fontSize: '38px', color: '#fff', marginTop: '4px' }}>{pct1(f.dominantProb)}</div>
          <div style={{ height: '5px', background: 'rgba(255,255,255,0.12)', borderRadius: '3px', marginTop: '10px', overflow: 'hidden' }}>
            <div style={{ height: '100%', width: `${f.dominantProb * 100}%`, background: T.accent, borderRadius: '3px' }} />
          </div>
          <div style={{ fontFamily: T.sans, fontSize: '11.5px', color: 'rgba(198,210,224,0.6)', marginTop: '10px', lineHeight: 1.4, textAlign: 'right' }}>{f.confRationale}</div>
        </div>
      </div>
    </section>
  );
  ...
  return (
    <Panel title="Scenario distribution" meta={`${f.scenarios.length} scenarios · blended vs deterministic vs historical`}>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '14px' }}>
        {f.scenarios.map((s, i) => {
          const top = i === 0 ? T.accent : s.blended >= 0.15 ? T.up : T.dn;
          return (
            <div key={s.id} style={{ ...sx.subPanel, padding: '18px', borderTop: `3px solid ${top}` }}>
              <div style={{ ...sx.monoVal, fontSize: '30px', color: T.text }}>{pct1(s.blended)}</div>
              <div style={{ fontFamily: T.sans, fontSize: '14px', fontWeight: 600, color: T.text, margin: '8px 0', lineHeight: 1.25 }}>{s.label}</div>
              <p style={{ margin: '0 0 12px', fontFamily: T.sans, fontSize: '12px', color: T.textSub, lineHeight: 1.45, minHeight: '68px' }}>{s.desc}</p>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontFamily: T.mono, fontSize: '11px', color: T.textMuted, borderTop: `1px solid ${T.borderSub}`, paddingTop: '9px' }}>
                <span>det {pct1(s.det)}</span><span>hist {pct1(s.hist)}</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontFamily: T.mono, fontSize: '11px', color: T.textMuted, paddingTop: '5px' }}>
...
function FanChart({ f }: { f: Forecast }) {
  ...
  const band = (lo: 'p10' | 'p25', hi: 'p90' | 'p75', op: number) => {
    ...
    return <path d={`M${top} L${bot} Z`} fill={T.accent} opacity={op} />;
  };
    ...
    <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1.5fr) minmax(0, 1fr)', gap: '20px' }} className="helix-fan-grid">
      <Panel title="Forward return distribution" meta="analogue-weighted SPY paths">
        <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%' }}>
          <line x1={padL} y1={y(0)} x2={W - padR} y2={y(0)} stroke={T.border} strokeDasharray="3 3" />
          {band('p10', 'p90', 0.14)}
          {band('p25', 'p75', 0.26)}
          <path d={medPath} fill="none" stroke={T.accentDark} strokeWidth={2.5} />
          {fan.map((d, i) => <circle key={d.h} cx={xs[i]} cy={y(d.med)} r={3} fill={T.accentDark} />)}
          {[mn, 0, mx].map((t) => (
            <text key={t} x={6} y={y(t) + 4} fill={T.textMuted} fontSize={10} fontFamily={T.mono}>{t > 0 ? '+' : ''}{t.toFixed(0)}%</text>
          ))}
          {fan.map((d, i) => (
            <text key={d.h} x={xs[i]} y={H - 8} fill={T.textMuted} fontSize={10} textAnchor="middle" fontFamily={T.mono}>{d.h}</text>
          ))}
        </svg>
        <div style={{ display: 'flex', gap: '18px', flexWrap: 'wrap', fontFamily: T.sans, fontSize: '11.5px', color: T.textSub, marginTop: '8px' }}>
          <LegendSwatch color={`${T.accent}26`} label="p10–p90" />
          <LegendSwatch color={`${T.accent}42`} label="p25–p75" />
          <LegendSwatch color={T.accentDark} label="median" />
        </div>
      </Panel>
...
function LegendSwatch({ color, label }: { color: string; label: string }) {
  return <span style={{ display: 'flex', alignItems: 'center', gap: '6px' }}><span style={{ width: '12px', height: '12px', borderRadius: '3px', background: color }} />{label}</span>;
}
function RiskCell({ k, v, suffix, sub }: { k: string; v?: number; suffix: string; sub: string }) {
  const color = v === undefined ? T.text : v > 0 ? T.up : T.dn;
  return (
    <div style={{ ...sx.subPanel, padding: '16px' }}>
      <div style={{ fontFamily: T.mono, fontSize: '10.5px', letterSpacing: '0.1em', color: T.textMuted, marginBottom: '8px' }}>{k}</div>
      <div style={{ ...sx.monoVal, fontSize: '26px', color }}>{v === undefined ? '—' : `${v > 0 ? '+' : ''}${v}${suffix}`}</div>
      <div style={{ fontFamily: T.sans, fontSize: '11px', color: T.textMuted, marginTop: '4px' }}>{sub}</div>
    </div>
  );
}
    ...
    <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1.5fr) minmax(0, 1fr)', gap: '20px' }} className="helix-fan-grid">
      <Panel title="Positioning read">
        <div style={{ marginBottom: '18px' }}>
          <Chip label="PREFERRED" color={T.up} />
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', marginTop: '10px' }}>
            {f.preferred.map((x) => <Chip key={x} label={x} />)}
          </div>
        </div>
        <div>
          <Chip label="AVOID" color={T.dn} />
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', marginTop: '10px' }}>
      ...
      <Panel title="Key tensions">
        {f.tensions.map((t, i) => (
          <div key={i} style={{ display: 'flex', gap: '12px', padding: '12px 0', borderTop: i ? `1px solid ${T.borderSub}` : 'none' }}>
            <span style={{ fontFamily: T.mono, fontSize: '12px', color: T.accentDark, paddingTop: '2px' }}>0{i + 1}</span>
            <span style={{ fontFamily: T.sans, fontSize: '13.5px', color: T.textSub, lineHeight: 1.5 }}>{t}</span>
          </div>
        ))}
      </Panel>
    </div>
...
// ─────────────────────────────────────────────────────────────
const LAYER_FILTERS = ['All', 'Monetary & Liquidity', 'Credit & Stress', 'Volatility Structure', 'Breadth & Participation', 'Positioning & Sentiment', 'Commodities', 'Earnings'];
  ...
  return (
    <Panel title="Macro indicators" meta={`${f.indicators.length} model inputs`}>
        ...
        {LAYER_FILTERS.map((L) => (
          <button
            ...
            style={{
              ...
              background: layer === L ? T.accentSoft : T.surface,
              color: layer === L ? T.accentDark : T.textSub,
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: '12px' }}>
        {items.map((ind, idx) => {
          const key = ind.label + idx;
          const isExp = expanded === key;
          const valStr = typeof ind.val === 'number' ? ind.val : String(ind.val).slice(0, 18);
          return (
            <div
              key={key}
              onClick={() => setExpanded(isExp ? null : key)}
              style={{
                ...sx.subPanel, padding: '16px', cursor: 'pointer',
                gridColumn: isExp ? '1 / -1' : undefined,
              }}
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '8px', marginBottom: '12px' }}>
                <div>
                  <div style={{ fontFamily: T.sans, fontSize: '13px', fontWeight: 600, color: T.text, lineHeight: 1.25 }}>{ind.label}</div>
                  <div style={{ fontFamily: T.mono, fontSize: '9.5px', letterSpacing: '0.06em', color: T.textMuted, marginTop: '3px', textTransform: 'uppercase' }}>{ind.cat} · {ind.role.replace(/_/g, ' ')}</div>
                </div>
                <Chip label={ind.signal} color={signalColor(ind.signal)} />
              </div>
              <div style={{ ...sx.monoVal, fontSize: '24px', color: T.text }}>{valStr}</div>
              <Sparkline seed={hashStr(ind.label)} color={signalColor(ind.signal)} tall={isExp} />
              <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '12px', fontFamily: T.mono, fontSize: '10.5px', color: T.textMuted }}>
                <span>{ind.trend}</span><span>conf {ind.conf ?? '—'}</span>
              </div>
            </div>
          );
        })}
...
function Sparkline({ seed, color, tall }: { seed: number; color: string; tall?: boolean }) {
  ...
  const d = series.map((p, i) => `${(i / (n - 1)) * W},${H - ((p - mn) / rng) * (H - 6) - 3}`).join(' L');
  return (
    <svg viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="none" style={{ width: '100%', height: tall ? '120px' : '44px', marginTop: '12px' }}>
  ...
  if (!result) {
    return (
      <Panel title="Market narrative · SPY">
        <div style={{ padding: '28px 8px', fontFamily: T.sans, fontSize: '13px', color: T.textMuted }}>
    ...
    <>
      ...
      <Panel title="Market narrative · SPY" meta="Reality · story · price">
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px', marginBottom: snap.bullets.length ? '18px' : 0 }}>
          {snap.regimeTone ? <SnapChip label="Regime Tone" value={snap.regimeTone} accent={STANCE_COLOR[snap.regimeTone.toLowerCase()] ?? T.textMuted} /> : null}
          {snap.primaryGap ? <SnapChip label="Primary Gap" value={snap.primaryGap} accent={T.wa} /> : null}
          {snap.primaryArchetype ? <SnapChip label="Primary Archetype" value={snap.primaryArchetype} accent={T.accent} /> : null}
        ...
        {snap.bullets.map((b) => (
          <div key={b.label} style={{ display: 'flex', gap: '12px', alignItems: 'flex-start', marginBottom: '10px' }}>
            <span style={{ flexShrink: 0, width: '60px', fontFamily: T.sans, fontSize: '10px', letterSpacing: '0.12em', textTransform: 'uppercase', fontWeight: 700, marginTop: '4px', color: b.label === 'Reality' ? T.up : b.label === 'Story' ? T.accent : T.navy }}>{b.label}</span>
            <p style={{ margin: 0, fontFamily: T.sans, fontSize: '14px', color: T.text, lineHeight: 1.6 }}>{b.text}</p>
          </div>
        ))}
      </Panel>

      {/* Dominant themes + inefficiency map */}
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) minmax(0, 1fr)', gap: '20px' }} className="helix-fan-grid">
        <Panel title="Dominant themes" meta={`${themes.length} theme${themes.length !== 1 ? 's' : ''}`}>
          {themes.length === 0 ? (
            <div style={{ padding: '20px 4px', fontFamily: T.sans, fontSize: '13px', color: T.textMuted }}>No dominant themes identified.</div>
          ) : (
        ...
        <Panel title="Inefficiency map">
          {inefficiency.length === 0 ? (
            <div style={{ padding: '20px 4px', fontFamily: T.sans, fontSize: '13px', color: T.textMuted }}>No explicit inefficiency classifications in this run.</div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              {inefficiency.map((row, i) => (
                <div key={i} style={{ ...sx.subPanel, padding: '12px 14px', borderLeft: `3px solid ${T.wa}` }}>
                  <div style={{ display: 'flex', gap: '10px', marginBottom: '8px' }}>
                    <span style={{ fontFamily: T.sans, fontSize: '11px', color: T.textMuted, paddingTop: '2px' }}>{i + 1}.</span>
                    <span style={{ fontFamily: T.sans, fontSize: '13.5px', fontWeight: 600, color: T.text, lineHeight: 1.4 }}>{row.subject}</span>
                  </div>
                  {row.gap ? <div style={{ fontFamily: T.sans, fontSize: '12.5px', color: T.textSub, lineHeight: 1.5, marginBottom: row.archetype || row.confidence !== null ? '8px' : 0 }}>{row.gap}</div> : null}
      ...
      {watch.length ? (
        <Panel title="Watchpoints">
          ...
          <ol style={{ margin: 0, paddingLeft: '20px' }}>
            {watch.map((w, i) => (
              <li key={i} style={{ fontFamily: T.sans, fontSize: '13.5px', color: T.text, lineHeight: 1.55, marginBottom: '6px' }}>{w}</li>
            ))}
          </ol>
        </Panel>
      ) : null}
...
function SnapChip({ label, value, accent }: { label: string; value: string; accent?: string }) {
  return (
    <div style={{ padding: '12px 14px', background: T.surfaceMuted, border: `1px solid ${T.borderSub}`, borderRadius: '10px', borderLeft: accent ? `3px solid ${accent}` : `1px solid ${T.borderSub}` }}>
      <div style={{ fontFamily: T.sans, fontSize: '10px', letterSpacing: '0.12em', textTransform: 'uppercase', color: T.textMuted, fontWeight: 600, marginBottom: '4px' }}>{label}</div>
      <div style={{ fontFamily: T.sans, fontSize: '13.5px', color: T.text, fontWeight: 500, lineHeight: 1.45 }}>{value}</div>
    </div>
  );
}
  ...
  const Row = ({ label, color, value }: { label: string; color: string; value: string }) =>
    value ? (
      <div style={{ display: 'flex', gap: '10px', marginBottom: '6px' }}>
        <span style={{ fontFamily: T.sans, fontSize: '10px', letterSpacing: '0.1em', textTransform: 'uppercase', color, fontWeight: 700, width: '64px', flexShrink: 0, paddingTop: '3px' }}>{label}</span>
        <p style={{ margin: 0, fontFamily: T.sans, fontSize: '13px', color: T.textSub, lineHeight: 1.55, flex: 1 }}>{value}</p>
      </div>
    ) : null;
  return (
    <div style={{ ...sx.subPanel, padding: '16px 18px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '12px', marginBottom: '10px' }}>
        <span style={{ fontFamily: T.sans, fontSize: '14.5px', fontWeight: 600, color: T.text, lineHeight: 1.4 }}>{theme.title}</span>
        <div style={{ display: 'flex', gap: '6px', flexShrink: 0 }}>
          {theme.stance ? <Chip label={theme.stance} color={sc} /> : null}
          {theme.confidence !== null ? <Chip label={`${theme.confidence}/100`} /> : null}
        </div>
      </div>
      {theme.thesis ? <p style={{ margin: '0 0 12px', fontFamily: T.sans, fontSize: '13px', color: T.textSub, lineHeight: 1.55 }}>{theme.thesis}</p> : null}
      <Row label="Reality" color={T.up} value={theme.reality} />
      <Row label="Story" color={T.accent} value={theme.story} />
      <Row label="Price" color={T.navy} value={theme.price} />
      <Row label="Gap" color={T.wa} value={theme.gap} />
      <Row label="Falsifier" color={T.dn} value={theme.falsifier} />
    </div>
  );
}
    ...
    <main style={sx.main}>
      <div style={sx.pageShell}>
        <div>
          ...
          <h1 style={{ fontFamily: T.sans, fontSize: '32px', fontWeight: 700, color: T.text, margin: 0 }}>What matters now</h1>
          <div style={{ fontFamily: T.mono, fontSize: '12px', color: T.textMuted, marginTop: '8px' }}>Model run · {forecast.asof || '—'}</div>
        </div>
...
// ─────────────────────────────────────────────────────────────
// Embedded sample (from macro_forecast_2026-06-05). Lets the page render
// before FORECAST_ENDPOINT is live. Remove once the endpoint is wired.
// ─────────────────────────────────────────────────────────────
